...
export default function SellSignalDetailScreen() {
  ...
  const params = { positionId: position.id, symbol: position.symbol, name: position.instrumentName, price: String(position.currentPrice), side: 'SELL' };
  return (
    <Screen>
      <TitleBlock eyebrow="매도 조건 상태 · ACTIVE" title={position.instrumentName} body="8월 14일 완성 일봉 · 같은 봉의 충족 근거를 합친 알림" />
      <Banner title="자동 매도 주문이 아닙니다" body="실제 매도 체결을 직접 등록한 뒤에만 수량이 바뀝니다." />
      ...
      <Button label="부분 · 전량 매도 처리" onPress={() => router.push({ pathname: '/trade', params })} />
    </Screen>
  );
}
