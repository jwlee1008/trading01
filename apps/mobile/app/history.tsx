  ...
  return (
    <Screen>
      ...
      {closed.length === 0 ? <EmptyState symbol="□" title="종료 기록이 없어요" body="전량매도한 포지션이 이곳에 남습니다." action="열린 포지션" onAction={() => router.push('/holdings')} /> : closed.map((position) => {
        ...
        return <Surface key={position.id} style={{ gap: spacing.sm }}><View style={{ flexDirection: 'row', justifyContent: 'space-between' }}><AppText variant="bodyStrong">{position.instrumentName}</AppText><Chip label="CLOSED" /></View><AppText tone={realizedProfit >= 0 ? 'positive' : 'negative'} variant="subtitle">실현 {formatWon(realizedProfit)}</AppText><AppText variant="caption" tone="muted">체결 {position.executions.length}건 · 잔여 0주 · 감시 종료</AppText></Surface>;
      })}
    </Screen>
  );
}
